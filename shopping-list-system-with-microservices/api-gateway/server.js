...
class APIGateway {
    ...
    async getDashboard(req, res) {
        try {
            ...
            const [userResponse, productsResponse, categoriesResponse, listsResponse] = await Promise.allSettled([
                ...
                this.callService('list-service', '/lists', 'GET', authHeader, { limit: 10 })
            ]);
    ...
    async globalSearch(req, res) {
        try {
            ...
            // Buscar em produtos, usuários e listas (se autenticado)
            const authHeader = req.header('Authorization');
            const searches = [
                this.callService('product-service', '/search', 'GET', null, { q })
            ];

            // Adicionar busca de usuários e listas se autenticado
            if (authHeader) {
                searches.push(
            ...
            const [productResults, userResults, listResults] = await Promise.allSettled(searches);
    ...
    start() {
        this.app.listen(this.port, () => {
            ...
            console.log('   POST /api/users/auth/register');
