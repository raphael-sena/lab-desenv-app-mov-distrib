...
class UserService {
        ...
        this.app.get('/', (req, res) => {
            res.json({
                ...
                endpoints: [
                    ...
                    'GET /users/:id/preferences - Buscar preferências do usuário',
                    'PUT /users/:id/preferences - Atualizar preferências (defaultStore, currency, theme, language)',
                    'GET /search - Buscar usuários por texto'
                ]
            });
        });
    ...
    async updateUserPreferences(req, res) {
        try {
            ...
            // Verificar permissão (usuário só pode alterar suas próprias preferências)
            if (req.user.id !== id && req.user.role !== 'admin') {
                return res.status(403).json({
                    success: false,
                    message: 'Acesso negado'
            ...
            // Preferências de interface (mantido por compatibilidade)
            if (theme) updates['profile.preferences.theme'] = theme;
            if (language) updates['profile.preferences.language'] = language;
            ...
            if (Object.keys(updates).length === 0) {
                return res.status(400).json({
                    success: false,
                    message: 'Nenhuma preferência fornecida para atualização'
                });
            }
    ...
    async getUserPreferences(req, res) {
        try {
            ...
            // Verificar permissão (usuário só pode ver suas próprias preferências)
            if (req.user.id !== id && req.user.role !== 'admin') {
                return res.status(403).json({
                    success: false,
                    message: 'Acesso negado'
            ...
            // Retornar apenas preferências organizadas
